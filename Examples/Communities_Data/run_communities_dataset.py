#!/usr/bin/env python3
"""
Communities Dataset Experiment Runner

This script runs experiments on the Communities dataset for 5 seeds using the BiasCorrected patch strategy.
...
def load_communities_dataset():
    """Load and preprocess the Communities dataset."""
    print("Loading Communities dataset...")
    
    # Load dataset
    dataset_path = os.path.join('..', '..', 'datasets', 'communities_cleaned.csv')
    ...
    print(f"  Target distribution: {y.describe().to_dict()}")
...
def run_communities_experiment():
    """Run experiments on Communities dataset for 5 seeds."""
    print("Communities Dataset Experiment Runner")
    print("=" * 60)
    print(f"Starting experiments at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    
    # Load dataset
    X, y, is_classification = load_communities_dataset()
    ...
    for seed in range(seeds):
        ...
        try:
            ...
            initial_voting_metrics['Dataset'] = 'Communities_Data'
        ...
        try:
            ...
            rocile_metrics['Dataset'] = 'Communities_Data'
        ...
        try:
            ...
            adarocile_metrics['Dataset'] = 'Communities_Data'
    ...
    if all_results:
        ...
        results_file = 'Communities_Data_results.csv'
        results_df.to_csv(results_file, index=False, float_format='%.6f')
        
        # Save summary statistics
        summary_file = 'Communities_Data_summary.txt'
        with open(summary_file, 'w') as f:
            f.write("Communities Dataset - Experiment Summary\n")
            f.write("=" * 50 + "\n")
