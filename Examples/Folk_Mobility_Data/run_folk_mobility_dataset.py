...
def load_folk_mobility_dataset():
    """Load and preprocess the Folk Mobility dataset."""
    print("Loading Folk Mobility dataset...")
    
    # Load dataset
    dataset_path = os.path.join('..', '..', 'datasets', 'folks_mobility_FL_cleaned.csv')
...
def run_folk_mobility_experiment():
    """Run experiments on Folk Mobility dataset for 5 seeds."""
    print("Folk Mobility Dataset Experiment Runner")
    print("=" * 60)
    print(f"Starting experiments at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    
    # Load dataset
    X, y, is_classification = load_folk_mobility_dataset()
    ...
    for seed in range(seeds):
        ...
        try:
            ...
            initial_voting_metrics['Dataset'] = 'Folk_Mobility_Data'
        ...
        try:
            ...
            rocile_metrics['Dataset'] = 'Folk_Mobility_Data'
        ...
        try:
            ...
            adarocile_metrics['Dataset'] = 'Folk_Mobility_Data'
    ...
    if all_results:
        ...
        results_file = 'Folk_Mobility_Data_results.csv'
        results_df.to_csv(results_file, index=False, float_format='%.6f')
        
        # Save summary statistics
        summary_file = 'Folk_Mobility_Data_summary.txt'
        with open(summary_file, 'w') as f:
            f.write("Folk Mobility Dataset - Experiment Summary\n")
            f.write("=" * 50 + "\n")
