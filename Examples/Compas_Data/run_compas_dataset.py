#!/usr/bin/env python3
"""
COMPAS Dataset Experiment Runner

This script runs experiments on the COMPAS dataset for 5 seeds using the BiasCorrected patch strategy.
...
def load_compas_dataset():
    ...
    # Load dataset
    dataset_path = os.path.join('..', '..', 'datasets', 'compas_cleaned.csv')
...
def run_compas_experiment():
    """Run experiments on COMPAS dataset for 5 seeds."""
    print("COMPAS Dataset Experiment Runner")
    print("=" * 60)
    print(f"Starting experiments at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    
    # Load dataset
    X, y, is_classification = load_compas_dataset()
    ...
    for seed in range(seeds):
        ...
        try:
            ...
            initial_voting_metrics['Dataset'] = 'Compas_Data'
        ...
        try:
            ...
            rocile_metrics['Dataset'] = 'Compas_Data'
        ...
        try:
            ...
            adarocile_metrics['Dataset'] = 'Compas_Data'
    ...
    if all_results:
        ...
        results_file = 'Compas_Data_results.csv'
        results_df.to_csv(results_file, index=False, float_format='%.6f')
        
        # Save summary statistics
        summary_file = 'Compas_Data_summary.txt'
        with open(summary_file, 'w') as f:
            f.write("COMPAS Dataset - Experiment Summary\n")
            f.write("=" * 50 + "\n")
