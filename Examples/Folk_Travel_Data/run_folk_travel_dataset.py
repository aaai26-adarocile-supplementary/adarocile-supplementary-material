...
def load_folk_travel_dataset():
    """Load and preprocess the Folk Travel dataset."""
    print("Loading Folk Travel dataset...")
    
    # Load dataset
    dataset_path = os.path.join('..', '..', 'datasets', 'folks_travel_FL_cleaned.csv')
...
def run_folk_travel_experiment():
    """Run experiments on Folk Travel dataset for 5 seeds."""
    print("Folk Travel Dataset Experiment Runner")
    print("=" * 60)
    print(f"Starting experiments at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    
    # Load dataset
    X, y, is_classification = load_folk_travel_dataset()
    ...
    for seed in range(seeds):
        ...
        try:
            ...
            initial_voting_metrics['Dataset'] = 'Folk_Travel_Data'
        ...
        try:
            ...
            rocile_metrics['Dataset'] = 'Folk_Travel_Data'
        ...
        try:
            ...
            adarocile_metrics['Dataset'] = 'Folk_Travel_Data'
    ...
    if all_results:
        ...
        results_file = 'Folk_Travel_Data_results.csv'
        results_df.to_csv(results_file, index=False, float_format='%.6f')
        
        # Save summary statistics
        summary_file = 'Folk_Travel_Data_summary.txt'
        with open(summary_file, 'w') as f:
            f.write("Folk Travel Dataset - Experiment Summary\n")
            f.write("=" * 50 + "\n")
